...
<h1>Campaignresponses Form</h1>
   ...
   include('database-connection.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO campaignresponses (ResponseID,CampaignID, CustomerID, ResponseType, ResponseDate) VALUES (? ,?, ?, ?, ?)");
        $stmt->bind_param("issss", $ResponseID, $CampaignID, $CustomerID, $ResponseType, $ResponseDate);

        // Set parameters from POST data with validation (optional)
        $ResponseID = intval($_POST['Cus_Id']); // Ensure integer for ID
        $CampaignID = htmlspecialchars($_POST['Ft_Nm']); // Prevent XSS
        $CustomerID = htmlspecialchars($_POST['Lst_Nm']); // Prevent XSS
        $ResponseType = filter_var($_POST['eml']); 
        $ResponseDate = filter_var($_POST['phn']); 
...
// SQL query to fetch data from campaignresponses table
$sql = "SELECT * FROM campaignresponses";
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of campaignresponses</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of campaignresponses</h2></center>
    <table border="5">
        <tr>
     ...
     include('database-connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $cuid = $row['ResponseID']; // Fetch the Customer_Id
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_campaignresponses.php?ResponseID=$cuid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_campaignresponses.php?ResponseID=$cuid'>Update</a></td> 
                </tr>";
            }
        } else {
