...
<h1>interactions Form</h1>
   ...
   include('database-connection.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO interactions (InteractionID,CustomerID, Type, Date, Description) VALUES (? ,?, ?, ?, ?)");
        $stmt->bind_param("issss", $InteractionID, $CustomerID, $Type, $Date, $Description);

        // Set parameters from POST data with validation (optional)
        $InteractionID = intval($_POST['Cus_Id']); // Ensure integer for ID
        ...
        $Description = filter_var($_POST['phn']); 
...
<body>
    <center><h2>Table of interactions</h2></center>
    <table border="5">
        <tr>
     ...
     include('database-connection.php');
        ...
        // Prepare SQL query to retrieve interactions.
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $cuid = $row['InteractionID']; // Fetch the InteractionID
                echo "<tr>
