...
<h1>orders Form</h1>
   ...
   include('database-connection.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO orders (OrderID,CustomerID, OrderDate, TotalAmount) VALUES (? ,?, ?, ?)");
        $stmt->bind_param("isss", $OrderID, $CustomerID, $OrderDate, $TotalAmount);

        // Set parameters from POST data with validation (optional)
        $OrderID = intval($_POST['Cus_Id']); // Ensure integer for ID
        $CustomerID = htmlspecialchars($_POST['Ft_Nm']); // Prevent XSS
        $OrderDate = htmlspecialchars($_POST['Lst_Nm']); // Prevent XSS
        $TotalAmount = filter_var($_POST['eml']); 
...
<body>
    ...
    <table border="5">
     ...
     include('database-connection.php');
        ...
        // Prepare SQL query to retrieve orders.
        $sql = "SELECT * FROM orders";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $cuid = $row['OrderID']; // Fetch the OrderID
                echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_orders.php?OrderID=$cuid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_orders.php?OrderID=$cuid'>Update</a></td> 
                </tr>";
            }
        } else {
