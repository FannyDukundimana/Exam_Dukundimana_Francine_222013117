...
<h1>Products Form</h1>
   ...
   include('database-connection.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO products (ProductID,ProductName, Description, Price) VALUES (? ,?, ?, ?)");
        $stmt->bind_param("isss", $ProductID, $ProductName, $Description, $Price);

        // Set parameters from POST data with validation (optional)
        $ProductID = intval($_POST['Cus_Id']); // Ensure integer for ID
        $ProductName = htmlspecialchars($_POST['Ft_Nm']); // Prevent XSS
        $Description = htmlspecialchars($_POST['Lst_Nm']); // Prevent XSS
        $Price = filter_var($_POST['eml']); 
...
<body>
    ...
    <table border="5">
     ...
     include('database-connection.php');
        ...
        // Prepare SQL query to retrieve products.
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $cuid = $row['ProductID']; // Fetch the ProductID
                echo "<tr>
                    ...
                    <td>" . $row['Price'] . "</td>
                    <td><a style='padding:4px' href='delete_product.php?ProductID=$cuid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_product.php?ProductID=$cuid'>Update</a></td> 
                </tr>";
            }
        } else {
