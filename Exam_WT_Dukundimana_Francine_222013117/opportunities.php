...
<section>
<h1>Opportunities Form</h1>



     <form method="post" onsubmit="return confirmInsert();">
        <label for="OpportunityID">OpportunityID:</label>
        ...
        <input type="NUMBER" id="Lst_Nm" name="Lst_Nm" required><br><br>
   ...
   include('database-connection.php');
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO opportunities (OpportunityID,OpportunityName, Amount, CloseDate, Stage) VALUES (? ,?, ?, ?, ?)");
        $stmt->bind_param("issss", $OpportunityID, $OpportunityName, $Amount, $CloseDate, $Stage);

        // Set parameters from POST data with validation (optional)
        $OpportunityID = intval($_POST['Cus_Id']); // Ensure integer for ID
        $OpportunityName = htmlspecialchars($_POST['Ft_Nm']); // Prevent XSS
        $Amount = htmlspecialchars($_POST['Lst_Nm']); // Prevent XSS
        $CloseDate = filter_var($_POST['eml']); 
        $Stage = filter_var($_POST['phn']); 
...
<body>
    <center><h2>Table of opportunities</h2></center>
    <table border="5">
        <tr>
     ...
     include('database-connection.php');
        ...
        // Prepare SQL query to retrieve opportunities.
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $cuid = $row['OpportunityID']; // Fetch the OpportunityID
                echo "<tr>
                    ...
                    <td>" . $row['CloseDate'] . "</td>
                    <td>" . $row['Stage'] . "</td>
                    <td><a style='padding:4px' href='delete_interactions.php?InteractionID=$cuid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_interactions.php?InteractionID=$cuid'>Update</a></td> 
                </tr>";
            }
        } else {
